...
declare void @__kmpc_fork_call(%struct.ident_t*, i32, void (i32*, i32*, ...)*, ...) 
  ...
  auto ompFork = llvm::dyn_cast<OpenMPModel::Fork>(racefunc.at(0).get());
  REQUIRE(ompFork);
  CHECK(ompFork->getInst()->getCalledFunction()->getName() == "__kmpc_fork_call");
  CHECK(ompFork->getThreadEntry()->getName() == ".omp_outlined.");

  ompFork = llvm::dyn_cast<OpenMPModel::Fork>(racefunc.at(1).get());
  REQUIRE(ompFork);
  CHECK(ompFork->getInst()->getCalledFunction()->getName() == "__kmpc_fork_call");
  CHECK(ompFork->getThreadEntry()->getName() == ".omp_outlined.");

  auto ompJoin = llvm::dyn_cast<OpenMPModel::Join>(racefunc.at(2).get());
  REQUIRE(ompJoin);
  CHECK(ompJoin->getInst()->getCalledFunction()->getName() == "__kmpc_fork_call");

  ompJoin = llvm::dyn_cast<OpenMPModel::Join>(racefunc.at(3).get());
  REQUIRE(ompJoin);
  CHECK(ompJoin->getInst()->getCalledFunction()->getName() == "__kmpc_fork_call");
}
